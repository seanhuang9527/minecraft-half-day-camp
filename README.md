# Minecraft 麥塊程式半日營教案

這個 repository 保存可交接給授課老師使用的 Minecraft Education 半日營教案、MakeCode Python 程式、程式截圖及課程地圖。

## 使用方式

1. 先閱讀每一週資料夾內的 `lesson-plan.md`。
2. 依學生程度選擇低、中或高階程式，不要把三份程式疊在同一個 MakeCode 專案。
3. 將程式貼入 Minecraft Education 的 MakeCode Python 編輯器後，先測試再上課。
4. 地圖放在該週的 `maps/` 資料夾；程式截圖放在 `images/` 資料夾。

## 教案

| 週次 | 主題 | 時間 | 教案 |
|---|---|---|---|
| 梯次 A | 梯次A_迴圈：求生大冒險 | 09:00－12:00 | [查看教案](batch-a-survival-adventure/lesson-plan.md) |
| 梯次 B | 梯次B_迴圈：紅石工程師 | 09:00－12:00 | [查看教案](batch-b-redstone-engineer/lesson-plan.md) |
| 梯次 C | 梯次C_巢狀迴圈：怪物攻城戰 | 09:00－12:00 | [查看教案](batch-c-monster-siege/lesson-plan.md) |
| 梯次 D | 梯次D_巢狀迴圈：礦坑達人 | 09:00－12:00 | [查看教案](batch-d-mining-master/lesson-plan.md) |
| 梯次 E | 梯次E_巢狀迴圈：沙漠荒野 | 09:00－12:00 | [查看教案](batch-e-desert-wilderness/lesson-plan.md) |
| 梯次 F | 梯次F_條件判斷：紅石樂園 | 09:00－12:00 | [查看教案](batch-f-redstone-park/lesson-plan.md) |
| 梯次 G | 梯次G_迴圈與方向：怪物專家 | 09:00－12:00 | [查看教案](batch-g-monster-expert/lesson-plan.md) |
| 梯次 H | 梯次H_巢狀迴圈：新夢幻農場 | 09:00－12:00 | [查看教案](batch-h-dream-farm/lesson-plan.md) |
| 梯次 I | 梯次I_變數：狩獵大師 | 09:00－12:00 | [查看教案](week-01-catch-chicken/lesson-plan.md) |
| 梯次 K | 梯次K_巢狀迴圈：程式造橋闖關 | 09:00－12:00 | [查看教案](batch-k-nested-loop-bridge/lesson-plan.md) |
| 梯次 R | 梯次R_變數：挖礦挑戰賽 | 09:00－12:00 | [查看教案](batch-r-mining-challenge/lesson-plan.md) |
