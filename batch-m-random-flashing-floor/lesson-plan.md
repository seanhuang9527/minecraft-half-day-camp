...
今天Minecraft半日營的主題是「隨機數：閃動格子」。

孩子先認識「隨機數」，把它想像成由電腦進行的抽籤。程式每次會從1～4中選出一個數字，再將結果放進名為 `safe` 的數字盒子中。

接著，孩子使用條件判斷，把不同數字轉換成紅、黃、綠、藍四種安全顏色。完成進階程式後，程式還能在倒數結束時讓其他三種危險地板消失。

最後，孩子實際使用程式參加閃動格子挑戰，在遊戲中練習觀察、反應與程式除錯，也理解隨機數、變數及條件判斷如何共同控制遊戲規則。

今天每位孩子都非常投入，也順利完成自己的程式挑戰！
